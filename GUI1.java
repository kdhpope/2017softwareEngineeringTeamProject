...
public class GUI1 extends JFrame implements ActionListener {
	...
	public void InitLayout() {
		...
		rightScroll = new JScrollPane(rightText,
				JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
				JScrollPane.HORIZONTAL_SCROLLBAR_ALWAYS);
		leftScroll = new JScrollPane(leftText,
				JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
				JScrollPane.HORIZONTAL_SCROLLBAR_ALWAYS);
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource().equals(leftload)) {
			FileDialog dialog = new FileDialog(this, "Load For Left",
					FileDialog.LOAD);
			dialog.setDirectory(".");
			...
			FileDialog dialog = new FileDialog(this, "Load For Right",
					FileDialog.LOAD);
			dialog.setDirectory(".");
			dialog.setVisible(true);
			...
			StringTokenizer s = new StringTokenizer(GUI1.getLeftPanelText()); 
			while(s.hasMoreTokens()) {  
	            // nextToken 인자에는 분리할 토큰(분리할 스트링을 넣어줍니다)  
	            left.add(s.nextToken("\n"));  
	        }
			for(int i=0;i<10;i++){
